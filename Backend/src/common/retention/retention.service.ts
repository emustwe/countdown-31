 ...
 * otherwise grow without limit, so a daily job prunes them.
 */
@Injectable()
export class RetentionService {
  ...
  async cleanup(): Promise<void> {
    ...
    try {
      ...
      this.logger.log(
        `retention: resets=${resets.count} verifications=${verifications.count} throttles=${throttles.count} events=${events.count} tokens=${revokedTokens.count}`,
      );
    } catch (err) {
