...
function validateEnv(): void {
  const required = ["DATABASE_URL", "JWT_ACCESS_SECRET", "JWT_REFRESH_SECRET", "CRED_SECRET", "SOLANA_TREASURY_ADDRESS"];
  const missing = required.filter((k) => !process.env[k] || String(process.env[k]).length < 8);
  if (missing.length) {
    throw new Error(`Missing/weak required env vars: ${missing.join(", ")}. Set them before starting.`);
  }
  if (process.env.NODE_ENV === "production") {
    if (process.env.ALLOW_MOCK_MONEY === "true") throw new Error("ALLOW_MOCK_MONEY must NOT be true in production (it's a money faucet).");
    if (process.env.EMAIL_ENABLED !== "true") {
      // eslint-disable-next-line no-console
      console.warn("[config] EMAIL_ENABLED is not true — verification and password-reset emails will not be delivered.");
    }
  }
  ...
  if (looksMainnet && process.env.ALLOW_MAINNET !== "true") {
    throw new Error("Refusing to start against Solana mainnet without ALLOW_MAINNET=true (this moves real funds).");
  }
}
