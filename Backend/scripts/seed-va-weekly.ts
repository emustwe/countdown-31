/**
 * (Re)create a fresh "VA Weekly" VA-branded WEEKLY tournament with 98 dummy players seeded so that
 * GROUP 1 keeps the open seats and groups 2–10 are full. When the real user registers through the
 * UI, join() seats them into the first group with space — Group 1 — so they land where the action
 * starts first.
 *
 *   group 1     → 8 seeded + 2 OPEN  (you take one of these seats)
 *   groups 2-10 → 10 each (full)
 *   total seeded = 8 + 90 = 98
 *
 * VA rules: $10 entry fee, dynamic 2× prize pool, registration LOCKS 24h before Round 1. So Round 1
 * is scheduled ~2 days out — registration stays open for ~1 day so the user can join.
 *
 * Run:  npx tsx scripts/seed-va-weekly.ts
 */
import { PrismaClient } from "@prisma/client";

const ADMIN_EMAIL = "admin@auroraways.demo";
const NAME = "VA Weekly";
...
// Round 1 must be > 24h out or VA registration (locks 24h before) would already be closed.
...
async function main() {
  ...
  // 2. Ensure the dummy players exist (reuse across runs), each with a wallet.
  ...
  // 3. Create the tournament + rounds + STAGGERED groups (group g starts stride*g after the round
  //    start). Round start times are FIXED here and never recomputed.
  const now = Date.now();
  ...
  // 4. Seat 98 dummies so GROUP 1 keeps the open seats and groups 2-10 are full. Fill groups 2-10
  //    (indices 1..9) fully first, then group 1 (index 0) up to (per - OPEN) — leaving OPEN seats.
  const per = ROUNDS[0]!.playersPerGroup; // 10
  const OPEN = 100 - DUMMIES; // 2 open seats, kept in group 1
  let idx = 0;
  ...
  for (let s = 0; s < per - OPEN; s++) await seatOne(dummies[idx++]!, 0, s); // group 1: seats up to (per - OPEN)
  ...
  console.log("\nRound 1 group fill:");
  round.matches.forEach((m) => console.log(`  Group ${m.index + 1}: ${m.players.length}/${round.playersPerGroup}${m.players.length < round.playersPerGroup ? "  ← open seat(s)" : ""}`));
  const total = round.matches.reduce((a, m) => a + m.players.length, 0);
  console.log(`\nSeated ${total}/100 · ${100 - total} open slot(s) in Group 1 → register through the UI to take one.`);
