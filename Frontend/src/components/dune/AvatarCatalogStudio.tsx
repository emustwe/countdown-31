...
export function AvatarCatalogStudio() {
  ...
  // Styling: the wearable glasses OVERLAY (layered onto the cow's eyes), distinct from the catalog
  // hat/glasses variants — equipped from the "Styling" section.
  const stylingGlasses = useAvatarStore((state) => state.stylingGlasses);
  const setStylingGlasses = useAvatarStore((state) => state.setStylingGlasses);
  ...
  const [previewGlasses, setPreviewGlasses] = useState(stylingGlasses);
  ...
  const isEquipped =
    ...
    stylingGlasses === previewGlasses;

  useEffect(() => {
    setPreviewId(current.id);
    setPreviewBackgroundId(backgroundId);
    setPreviewFrameId(frameId);
    setPreviewGlasses(stylingGlasses);
  }, [backgroundId, current.id, frameId, stylingGlasses]);
  ...
  function equip() {
    ...
    setStylingGlasses(previewGlasses);
