...
export function MobileLandscape() {
  ...
  useEffect(() => {
    ...
    const orientation = (window.screen as Screen & { orientation?: { lock?: (o: string) => Promise<void>; unlock?: () => void } }).orientation;
  ...
  return (
    <div className="rotate-overlay" role="dialog" aria-label="Rotate your device">
      ...
      <p>
        <FlipText
          intervalMs={4200}
          items={[<>Turn your phone sideways to play in landscape.</>, <>가로 모드로 플레이하려면 휴대폰을 옆으로 돌리세요.</>]}
        />
      </p>
    </div>
  );
